...
<html>
	<head>
		...
		<link rel="stylesheet" href="ol-layerswitcher/src/ol-layerswitcher.css" />
...
?>
		...
		<script>
			...
			$(document).ready(function() {
				...
				var markersArray = [];
				// Pole jmen míst mi bude držet informace o nazvech míst
				var place_names = [];
				...
				map.on('click', function(evt){
					...
					// pokud klik na marker, tak nechci přidat další ale chci zobrazit popup

					// Pokud klik na nejaký marker, tak tahle funkce jej zachytí
				...
				function addMarker(evt, coor) {
					...
					// Kdyz se nahravaji body z importu evt parametr je null a musi se coordinates nahrat jinak
					if(evt != null) {
						var mouseCoords = ol.proj.transform(evt.coordinate, 'EPSG:3857', 'EPSG:4326');
					} else {
						var mouseCoords = coor;
					}

					// Tady se kazdemu markeru prida atribut name, ktery drzi index v poli place_names
					...
					//  do pole jmen se přidá prázdný string
					place_names.push("");
					...
					// Vytvoří se dictionary, ktera drží informace o řádku
					// uid pomaha s vazbou na jmena bodu do popupu
					var formLine = {
						"lat": lat,
						"lon": lon,
						"place_name": "",
					...
					// unshift přidá novy řádek na první místo v poli, dá se použít push
					// ale pro další úkony je lepší když bude nejnovější udaj na prvním
					// místě pole
					formLines.unshift(formLine);

					// zavolej funkci, která upraví formulář
					updateForm(formLines);
				}

				// Funkce, upravující formulář pro daný počet markerů
				function updateForm(lines) {

					// Uloží se instance těla tabulky, kde je formulář
					var tbody = $(".location-wrapper");

					// Pomocná proměnná kde se formuje obsah formuláře
					var form = "";

					// Tohle je JQuery funkce, kteraa funguje jako foreach smyčka
					// vezme to kazdy zaznam v dictionary lines a ve funkci se da
					// dělat co chci s kazdym řádkem

					// Vytvoren string pro každý řádek formuláře a přidán do 
					// pomocné proměnné
					// Kazdemu bodu pridan i uid pro ucely popupu
					...
					$.each(lines, function(key, val) {
						...
						form +=	'<td><input class="place_name" type="text" name="name'+key+'" value="'+val["place_name"]+'" id="name'+key+'" placeholder="Place name" ></td>';
					...
					// Pomocí této funkce se vloží vytvořený formulář do těla tabulky/formuláře
					tbody.html(form);
				...
				// Tahle funkce navazuje event listener na delete tlačítka. 
				// Řádky a celý formulář je přidáván dynamicky a to znamená, že aby se na tyto
				// elementy navázalo nejakou událost/event listener, tak se musí použít funknce a jeji
				// konstrukce, jakou vidíme tady. Začíná se elementem, který je stálý, nemění se.
				// Což je v tomto případě html, ale daji se použít třeba i body nebo nějaky parent element
				// formuláře. 
				$("html").on("click", ".delete-btn", function(e) {
					e.preventDefault();

					// Sežeň id řádku údaje, který se má smazat.
					// Při vytváření formuláře se přidal data-id attribute každému cancel tlačítku
					...
					// Stejně tak musí být v poli markerů vymazán odpovídající marker k dané pozici
				...
				// Tady se pošle formulář pomocí ajaxu a post methody
				// kvůli kumulativnímu způsobu těch více bodu je tohle  lepší 
				// cesta než jako čiste pres form submit
				$(".form-inline").on("submit", function(e) {
					...
					// Ajax funkce, metoda post protože se posilaji data ze frontendu do databaze
					// data jako Json, takze se vezme pole formLine s pomoci
					// JSON.stringify funkce, z toho bude validní json format
					$.ajax({
						type: "POST",
						url: "./save.php",
						data: {
						...
						success: function(response) {
							// Tadyhle se naklada s odpovědí, kterou pošle php script
							// je to udelane tak, že odpověd je také v json a má dva klíče
							// status a message. Status bude true/false podle toho jestli se data uložila
							// správně. a v message bude informace,že bud vsechno dobre dopadlo
