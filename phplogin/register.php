...
if ($stmt = $con->prepare('SELECT id, password FROM accounts WHERE username = ? OR email = ?')) {
	...
	if ($stmt->num_rows > 0) {
		...
		echo 'Username or email already exists, please go back and choose another! '  ;
	} else {
		...
		if ($stmt = $con->prepare('INSERT INTO accounts (username, password, email, activation_code) VALUES (?, ?, ?, ?)')) {			
			...
			// Kdybychom chtěli registraci účtu potvrdit emailem vygenerujeme unikatni kod

			$uniqid = 'activated';
			// $stmt->bind_param('ssss', $_POST['username'], $password, $_POST['email'], $uniqid);

			// Kod nastavime jako activated
			$stmt->bind_param('ssss', $_POST['username'], $password, $_POST['email'], $uniqid);
			$stmt->execute();
